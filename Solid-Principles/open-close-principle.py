...
class SavetoMongo(SavetoDB):
  def __init__(self,cart):
    self.cart = cart
  
  def save(self):
    return f"{len(self.cart.totalProducts)} products saved to MongoDb"

class SavetoPostgres(SavetoDB):
  def __init__(self,cart):
    self.cart = cart
  
  def save(self):
    return f"{len(self.cart.totalProducts)} products saved to Postgres"
